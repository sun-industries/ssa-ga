...
extern "C" void setObserver(Observer observer) {
    observer = observer;
 ...
 * 
 */
extern "C" bool startRecording(Observer observer) {
    setObserver(observer);
    isRecording = true;
}

extern "C" bool endRecording() {
    isRecording = false;
}
...
extern "C" std::vector<TickResult> tick(int year, int  mon, int day, int hr, int mi, int sec)
{
    ...
    for(elsetrec& satrec : satrecs) {
        ...
        if(observer.defined) {
            ...
            lookAngles = ecfToLookAngles(observer, pos_ecf);
            ...
            if(lookAngles.elevation * (180.0/pi) >= observer.minElevation) {
                ...
                if(isRecording) {
                    ...
                    bool isRisingNow = lookAngles.elevation >= satItem._prevElevation;
                    ...
                    if(!satItem._isRising && isRisingNow) {
                        ...
                        auto maxElevation = std::max_element(
                            std::begin(satItem.transits[satItem._transitIndex].elevation), 
                            std::end(satItem.transits[satItem._transitIndex].elevation)
                        );
